...
export default function MyCourses() {
  ...
  const [ready, setReady]       = useState(false)   // true once data has loaded

  const userId = profile?.id || user?.id

  useEffect(() => {
    // Wait until we know userId and membership status before attempting load
    if (!userId) return
  ...
  // ── Access gate — only lock after membershipChecked (no false flash) ────────
  if (!hasAccess && membershipChecked) return (
    <div style={noAccessStyle}>
      ...
      <a href="https://whop.com/tikcshift/tickshift-premium-access/" target="_blank" rel="noopener noreferrer" style={ctaLinkStyle}>
        Get Access on Whop
      </a>
    </div>
  )

  // ── Shell renders immediately — header never waits on data ────────────────
  return (
    <div style={{ minHeight: '100vh', background: '#08162E', padding: '2rem 2.25rem', fontFamily: "'Open Sans', sans-serif" }}>
      ...
      {/* Course grid — skeletons while loading, cards when ready */}
      {!ready ? (
