...
export default function StudentDashboard() {
  const { profile, user, hasAccess } = useAuth()
  ...
  if (loading) return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100vh', background: '#08162E', gap: 14 }}>
  ...
  if (!hasAccess) return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', background: '#08162E', padding: '2rem', textAlign: 'center', gap: 16 }}>
      ...
      <p style={{ color: '#6E7B8F', fontSize: 14, maxWidth: 420, lineHeight: 1.6, margin: 0 }}>Your account doesn't have an active TickShift membership yet. Purchase access via Whop or contact support.</p>
    ...
    <div style={{ minHeight: '100vh', background: '#08162E', padding: '2rem 2.25rem', fontFamily: "'Open Sans', sans-serif" }}>
      <div style={{ background: 'linear-gradient(135deg, rgba(13,31,60,0.8), rgba(8,18,46,0.9))', border: '1px solid rgba(15,111,255,0.14)', borderRadius: 16, padding: '1.5rem 1.75rem', marginBottom: '1.75rem', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative' }}>
          ...
          <div style={{ fontFamily: "'Montserrat', sans-serif", fontWeight: 900, fontSize: 'clamp(20px, 2.5vw, 26px)', color: '#F8FFFF', marginBottom: 4 }}>{profile?.full_name?.split(' ')[0] || 'Trader'} <span style={{ color: '#3CCBFF' }}>—</span></div>
          <p style={{ fontSize: 12.5, color: '#6E7B8F', maxWidth: 480, marginBottom: '1rem' }}>Keep pushing. Every lesson brings you closer to consistency.{overallPct > 0 && ` You're ${overallPct}% through your current curriculum.`}</p>
          <button onClick={() => { const inProgress = courses.find(c => pct(c) > 0 && pct(c) < 100); const target = inProgress ?? courses[0]; if (target) navigate(`/courses/${target.id}`); else navigate('/courses') }} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '0.7rem 1.5rem', background: 'linear-gradient(135deg, #0F6FFF, #3CCBFF)', border: 'none', borderRadius: 10, color: '#fff', fontFamily: "'Montserrat', sans-serif", fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>
            ...
            {assignments.filter(a => !submissions.includes(a.id)).map((a, i, arr) => (
              <div key={a.id} style={{ padding: '1rem 1.25rem', borderBottom: i < arr.length - 1 ? '1px solid rgba(15,111,255,0.1)' : 'none', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
                <div>
                  ...
                  {a.courses && <span style={{ fontSize: 9.5, fontWeight: 700, letterSpacing: 0.8, background: 'rgba(15,111,255,0.15)', border: '1px solid rgba(60,203,255,0.25)', color: '#3CCBFF', borderRadius: 100, padding: '2px 9px' }}>{a.courses.title}</span>}
                </div>
